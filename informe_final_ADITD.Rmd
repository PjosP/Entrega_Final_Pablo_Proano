...
Vamos a realizar gráficos de cajas y bigotes para ver como se distribuyen los datos de las salidas y si existen atípicos.

```{r}
boxplot(
  base_seleccionada[, c("IVA", "ADVALOREM", "ICE_ADVALOREM", "FODINFA")],
...
```
El diagrama de cajas de las variables de salida evidencia una alta asimetría y una gran presencia de valores atípicos.

Esto podría impactar negativamente en nuestro estudio, por lo que vamos a proceder a eliminar esos datos atípicos.
...
filtro_IVA  <- eliminar_outliers_IQR(base_seleccionada$IVA)
filtro_ADV  <- eliminar_outliers_IQR(base_seleccionada$ADVALOREM)
filtro_ICE  <- eliminar_outliers_IQR(base_seleccionada$ICE_ADVALOREM)
filtro_FOD  <- eliminar_outliers_IQR(base_seleccionada$FODINFA)

filtro_total <- filtro_IVA & filtro_ADV & filtro_ICE & filtro_FOD

base_seleccionada_sin_outliers <- base_seleccionada[filtro_total, ]
...
```
Para ver como la función anterior impacto a nuestra base de datos, vamos a volver a obtener el diagrama de cajas:


```{r}
boxplot(
  base_seleccionada_sin_outliers[, c("IVA", "ADVALOREM", "ICE_ADVALOREM", "FODINFA")],
  main = "Diagrama de Cajas de las Variables de Salida",
  ylab = "Valor del tributo",
  col = c("lightblue", "lightgreen", "lightpink", "lightgray"),
  border = "black",
...
```
**Nota:** Después de la eliminación de Atípicos, la salida de mi estudio `ICCE_ADVALOREN` tienen toda una columna de ceros, por lo que la voy a eliminar de mi estudio.
...
# Creamos la nueva base de datos
nueva_base_seleccionada <- base_seleccionada_sin_outliers[, vars_seleccionadas]

# Verificación básica
dim(nueva_base_seleccionada)
```

## Analisis multivariante

Para saber si nuestro proyecto es Viable, voy a obtener la matriz de covarianzas, para analizar si las variables están relacionadas y podría predecir unas en función de otras.
...
vars_char <- names(nueva_base_seleccionada)[sapply(nueva_base_seleccionada, is.character)]

# Se creó una copia de la base original para trabajar con la versión numérica
base_numerica <- nueva_base_seleccionada
...
plot(test_FODINFA$FODINFA, pred_FODINFA,
     main = "Predicción vs Valor Real del FODINFA",
     xlab = "FODINFA Real",
